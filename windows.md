...
16. <a name="postgresql"></a>We will now install PostgreSQL. Close Hyper and open it again as administrator (like in step 9).
    ...
    ```bash
    choco install postgresql18 --yes --params '/Password:postgres'
    ```
    ...
    ```bash
    echo -e "\nexport PATH=\$PATH:\"/c/Program Files/PostgreSQL/18/bin\"" >> ~/.bash_profile
    echo "export PGDATA=\"/c/Program Files/PostgreSQL/18/data\"" >> ~/.bash_profile
...
7. To keep a history of things you have copied, clipboard managers like Ditto are an awesome option:
