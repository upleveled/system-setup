...
13. <a name="postgresql"></a>We will now install PostgreSQL. Copy each line in the following text, paste it in the terminal and hit return.
    ...
    ```bash
    [[ -d /opt/homebrew/var/postgresql@18 ]] && PGDATA_TMP=/opt/homebrew/var/postgresql@18 || PGDATA_TMP=/usr/local/var/postgresql@18
...
24. Click on Applications in the dock and start Slack. Log in to the UpLeveled Slack. Send your GitHub profile URL to [Karl](slack://user?team=TFFSPKL92&id=UFG252SH0). Also send your Google Account email address to Karl (if you haven't already).
...
12. To use AI agents on the command line, try Codex CLI (requires OpenAI paid account):
